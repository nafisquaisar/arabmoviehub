...
public class MovieDetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        View root = findViewById(R.id.movieDetailsRoot);
        if (root != null) {
    ...
    private void displayMovie(Movie movie) {
        ...
        rating.setText(String.format("★ %.1f / 10", movie.getVoteAverage()));
        String release = movie.getReleaseDate();
        if (release != null && !release.isEmpty()) {
            releaseDate.setText(release);
        } else {
            releaseDate.setText("Release date unavailable");
        }
        ...
        loadLocalImage(movie.getPosterPath(), poster);
        loadLocalImage(movie.getBackdropPath(), backdrop);
    }


    private void loadLocalImage(String imageName, ImageView imageView) {
        ...
        if (resourceId != 0) {
            Glide.with(this).load(resourceId).placeholder(R.drawable.placeholder_movie).error(R.drawable.placeholder_movie).into(imageView);
        } else {
            imageView.setImageResource(R.drawable.placeholder_movie);
        }
    }
